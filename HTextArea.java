 ...
 *
 * <p>Basé sur {@link JTextPane} (et non plus {@link javax.swing.JTextArea}) afin de
 * prendre en charge les styles par caractère : police, taille, gras, italique,
 * souligné, barré, couleur et casse.</p>
 *
 * <p>Toutes les méthodes de style agissent sur la <strong>sélection courante</strong>.
 * Si aucun texte n'est sélectionné, le style est mémorisé et s'appliquera
 * au prochain texte saisi (comportement standard de {@link JTextPane}).</p>
 *
 * @author FIDELE 
 * @see JTextPane
 */
public class HTextArea extends JTextPane {
    ...
    public HTextArea(String text) {
        ...
        updateUI();
        configureDefaults();
        // On passe par setText après init pour que le document soit prêt
        setText(text);
    }
     ...
     *
     * <p>Note : {@code setLineWrap} et {@code setWrapStyleWord} n'existent pas sur
     * {@link JTextPane}. Le retour à la ligne automatique est géré nativement
     * par son modèle de rendu (basé sur {@link javax.swing.text.WrappedPlainView}).</p>
     */
    ...
    /**
     * Installe le delegate d'interface utilisateur personnalisé.
     */
    @Override
     ...
     *
     * <p>Exemple : {@code setFontFamily("Arial")} ou {@code setFontFamily("Times New Roman")}</p>
     *
     ...
     *
     * <p>Équivalent du bouton "Effacer la mise en forme" de Word.</p>
     */
     ...
     *
     * @param italic {@code true} pour mettre en italique, {@code false} pour retirer
     */
    ...
    /**
     * Bascule le gras : si la sélection est déjà en gras, le retire — sinon l'active.
     *
     * <p>Pratique pour un bouton toggle dans une barre d'outils.</p>
     */
     ...
     *
     * <p>L'exposant élève le texte au-dessus de la ligne de base et le réduit
     * légèrement. Exemple : E = mc²  →  le "2" est en exposant.</p>
     *
     * <p>Note : exposant et indice sont mutuellement exclusifs. Activer l'un
     * désactive automatiquement l'autre.</p>
     *
     * @param superscript {@code true} pour activer, {@code false} pour désactiver
     */
    public void setSuperscript(boolean superscript) {
        SimpleAttributeSet attrs = new SimpleAttributeSet();
        StyleConstants.setSuperscript(attrs, superscript);
        // Désactiver l'indice pour éviter un état incohérent
        if (superscript) StyleConstants.setSubscript(attrs, false);
        appliquerAttribut(attrs);
    }
     ...
     *
     * <p>L'indice abaisse le texte sous la ligne de base et le réduit légèrement.
    ...
    public void setSubscript(boolean subscript) {
        ...
        if (subscript) StyleConstants.setSuperscript(attrs, false);
        appliquerAttribut(attrs);
    }

    /**
     * Bascule l'exposant sur la sélection courante.
     * Si déjà en exposant, le retire — sinon l'active.
     */
    ...
    /**
     * Bascule l'indice sur la sélection courante.
     * Si déjà en indice, le retire — sinon l'active.
     */
    ...
    /**
     * Applique une couleur de surlignage (fond du texte) à la sélection courante.
     *
     * <p>Équivalent du surlignage fluo de Word.</p>
     *
     ...
     *
     * <p>Exemple : "bonjour monde" → "Bonjour Monde"</p>
     */
     ...
     *
     * <p>Swing n'ayant pas de support natif des petites majuscules, l'effet est
     ...
     *
     * <p>L'effet est stocké comme attribut custom dans le {@link StyledDocument}
     ...
     *
     * <p>Passer {@link HTextEffect#NONE} supprime tout effet sur la sélection.</p>
     *
     ...
     *
     * <p>La couleur est utilisée différemment selon l'effet :</p>
     * <ul>
     ...
     *   <li>{@code EMBOSS} / {@code ENGRAVE} → la couleur est ignorée
     *       (les ombres blanc/noir sont fixes pour un rendu réaliste)</li>
     * </ul>
     *
     ...
     *
     * <p>Raccourci pour {@code setTextEffect(HTextEffect.NONE)}.</p>
     */
    ...
    /**
     * Retourne l'effet typographique actif au niveau du curseur (ou de la sélection).
     *
     * @return l'effet courant, ou {@link HTextEffect#NONE} si aucun n'est défini
     */
    public HTextEffect getSelectionTextEffect() {
        Object valeur = getInputAttributes().getAttribute(HEffectPainter.EFFECT_ATTRIBUTE);
        if (valeur instanceof HTextEffect effet) return effet;
    ...
    /**
     * Indique si le texte au niveau du curseur (ou de la sélection) est en gras.
     *
    ...
    /**
     * Indique si le texte au niveau du curseur (ou de la sélection) est en italique.
     *
    ...
    /**
     * Indique si le texte au niveau du curseur (ou de la sélection) est souligné.
     *
    ...
    /**
     * Applique un jeu d'attributs ({@link SimpleAttributeSet}) à la sélection courante.
     *
     * <p>C'est le point central par lequel passent toutes les méthodes de style.
     * Le paramètre {@code replace=false} signifie qu'on <em>fusionne</em> les nouveaux
     ...
     *
     * <p>Lit la taille actuelle de chaque caractère sélectionné pour calculer
     * la nouvelle valeur, en s'assurant qu'elle reste toujours ≥ 1 point.</p>
     *
     * @param delta la variation en points (positif = agrandir, négatif = réduire)
     */
    ...
    /**
     * Transforme le texte sélectionné selon une fonction de conversion de casse.
     *
     * <p>Après la transformation, les attributs de style (gras, couleur…) sont
     * préservés — seul le contenu textuel change.</p>
     *
     * @param convertisseur une fonction qui reçoit le texte brut et retourne
     *                      sa version transformée
     */
