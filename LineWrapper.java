 ...
 *
 * RESPONSABILITÉS PRÉCISES : 1. Organise une liste de composants en lignes
 * selon la largeur disponible 2. Calcule la hauteur de chaque ligne (maximum
 * des hauteurs des composants) 3. Positionne les composants horizontalement
 * avec espacement 4. Centre verticalement chaque composant dans sa ligne 5.
 * Calcule la hauteur totale requise pour un ensemble de dimensions prédéfinies
 * 6. Orchestre le layout complet des composants dans un groupe
 *
 * CONCEPTS IMPORTANTS : - Ligne (Line) : Groupe de composants qui tiennent sur
 * une même largeur - Wrapping : Retour à la ligne quand la largeur est
 * insuffisante - Espacement (Spacing) : Distance horizontale entre composants -
...
public class LineWrapper {
     ...
     *
     * Initialise un moteur de disposition en lignes sans configuration
     * spécifique
     */
    public LineWrapper() {
    ...
    // =========================================================================
    /**
     * Organise les composants en lignes avec retour automatique selon la
     * largeur
     *
     * ALGORITHME DE WRAPPING : 1. Parcourt les composants dans l'ordre 2. Pour
     * chaque composant, vérifie s'il tient dans la ligne actuelle 3. Si oui,
     * l'ajoute à la ligne actuelle 4. Sinon, termine la ligne actuelle et
    ...
    public List<List<Component>> wrapIntoLines(List<Component> components, int innerWidth, int spacing) {
        ...
        for (Component comp : components) {
            // OBTENTION DE LA TAILLE DU COMPOSANT
            Dimension pref = new Dimension(comp.getPreferredSize());
            ...
            if (currentLine.isEmpty() || currentLineWidth + requiredWidth <= innerWidth) {
                ...
                if (currentLine.isEmpty()) {
                    ...
                    // Ajout avec espacement : reqquireWidth = composant précédent + spacing
     ...
     *
     * PRINCIPE : La hauteur d'une ligne est la hauteur maximale parmi tous ses
     * composants. Cette hauteur détermine l'espace vertical nécessaire pour
     * afficher la ligne.
     *
     * CONTRAINTE THREAD : Doit être appelé sur l'Event Dispatch Thread (EDT)
     * car utilise Component.getPreferredSize()
     ...
     * @return Hauteur de la ligne en pixels (≥ 0) Retourne 0 si la ligne est
     * null ou vide
     *
     * @throws IllegalStateException si appelé hors de l'EDT
     */
     ...
     *
     * CARACTÉRISTIQUES : - Méthode purement algorithmique (n'utilise pas de
     * composants Swing) - Utile pour la planification du layout
     * (PreferredSizeCalculator) - Opère sur des Dimensions pré-calculées
     *
     ...
     *
     * ALGORITHME SPÉCIAL : - Si innerWidth ≤ 1 : Empilement vertical forcé (pas
     * de wrapping) - Sinon : Wrapping normal selon l'algorithme standard
     */
     ...
     *
     * ALGORITHME : 1. Parcourt les composants de la ligne dans l'ordre 2.
     * Positionne chaque composant avec l'espacement spécifié 3. Centre
     * verticalement chaque composant dans la hauteur de ligne
     *
     * CONTRAINTE THREAD : Doit être appelé sur l'Event Dispatch Thread (EDT)
    ...
    // =========================================================================
    /**
     * Dispose les composants dans un rectangle de groupe avec padding et
     * espacement
     *
     * ORCHESTRATION COMPLÈTE : 1. Calcule la zone interne disponible (après
     * padding) 2. Organise les composants en lignes avec wrapping 3. Pour
     * chaque ligne, calcule sa hauteur et la positionne 4. Gère le débordement
     * vertical (arrêt si plus d'espace)
     *
     * CONTRAINTE THREAD : Doit être appelé sur l'Event Dispatch Thread (EDT)
     *
...
public int computeDynamicMinWidth(
    ...
    // =========================================================================
    /**
     * Note: Vérifie que la méthode est appelée sur l'Event Dispatch Thread
     *
     * Swing est single-threaded : toutes les opérations sur les composants
     * doivent être effectuées sur l'EDT pour éviter les problèmes de
     * concurrence.
     *
     * @throws IllegalStateException si appelé hors de l'EDT
     *
