 ...
 *
 * Cette classe implémente l'interface GroupRenderer et fournit : 1. La
 * transformation des objets Java en composants Swing appropriés 2. La création
 * et configuration des en-têtes (headers) des groupes 3. La gestion de la
 * configuration hiérarchique (groupe → ruban → valeurs matérielles) 4.
 * L'orientation automatique des en-têtes selon leur position
 *
 * Architecture de configuration : - Niveau GROUPE (HRibbonGroup) :
 * Configuration spécifique par groupe (override) - Niveau RIBBON (Ribbon) :
 * Configuration globale par défaut - Niveau MATÉRIEL : Valeurs par défaut
 * codées en dur (fallback)
 *
...
public class DefaultGroupRenderer implements GroupRenderer {
    ...
    // =========================================================================
    /**
     * Transforme une valeur objet en composant Swing pour l'affichage dans un
     * groupe.
     *
     * Cette méthode examine le type de l'objet et délègue à la méthode de
     * création appropriée. Supporte plus de 20 types Java courants avec rendu
     * intelligent.
     *
     ...
     *
     * Cette méthode : 1. Récupère la configuration du groupe et du ruban 2.
     * Crée un HLabel adapté au type de valeur 3. Applique la configuration
     * hiérarchique (groupe → ruban) 4. Configure l'orientation selon la
    ...
    public Component getHeaderComponent(Ribbon ribbon, Object headerValue,
        ...
        // 5. CRÉATION DU PANEL CONTENEUR
        // On enveloppe le label dans un panel avec les boutons de déplacement
        JPanel headerPanel = new JPanel(new BorderLayout());
        headerPanel.setOpaque(false);

        // Bouton déplacer à gauche
        HButton btnLeft = new HButton("←");
//        btnLeft.setFocusable(false);
        btnLeft.setMargin(new Insets(0, 2, 0, 2));
        btnLeft.setPreferredSize(new Dimension(20, 16));
        btnLeft.setCornerRadius(0);
        // Bouton déplacer à droite  
        HButton btnRight = new HButton("→");
//        btnRight.setFocusable(false);
        btnRight.setMargin(new Insets(0, 2, 0, 2));
        btnRight.setPreferredSize(new Dimension(20, 16));
        btnRight.setCornerRadius(0);

        // Le label prend l'espace central
        headerLabel.setHorizontalAlignment(SwingConstants.CENTER);

        headerPanel.add(btnLeft, BorderLayout.WEST);
        headerPanel.add(headerLabel, BorderLayout.CENTER);
        headerPanel.add(btnRight, BorderLayout.EAST);

        // 6. LOGIQUE DE DÉPLACEMENT
        Object groupId = group != null ? group.getGroupIdentifier() : null;

        btnLeft.addActionListener(e -> {
            if (ribbon != null && ribbon.getGroupModel() != null && groupId != null) {
                HRibbonGroupModel gm = ribbon.getGroupModel();
                // Retrouver l'index ACTUEL du groupe via son identifiant
                int currentIndex = gm.getGroupIndex(groupId);
                if (currentIndex > 0) {
                    HRibbonGroup prevGroup = gm.getHRibbonGroup(currentIndex - 1);
                    if (prevGroup != null && !prevGroup.isCollapsed()) {
                        HRibbonLayoutManager lm = (HRibbonLayoutManager) ribbon.getLayout();
                        if (lm != null) {
                            lm.invalidateAllHeaders();
                            lm.invalidateLayout(ribbon);
                        }
                        ribbon.moveGroup(currentIndex, currentIndex - 1);
                    }
                }
            }
        });
        ...
        btnRight.addActionListener(e -> {
            if (ribbon != null && ribbon.getGroupModel() != null && groupId != null) {
                ...
                // Retrouver l'index ACTUEL du groupe via son identifiant
                int currentIndex = gm.getGroupIndex(groupId);
                if (currentIndex < gm.getGroupCount() - 1) {
                    HRibbonGroup nextGroup = gm.getHRibbonGroup(currentIndex + 1);
                    if (nextGroup != null && !nextGroup.isCollapsed()) {
                        HRibbonLayoutManager lm = (HRibbonLayoutManager) ribbon.getLayout();
                        if (lm != null) {
                            lm.invalidateAllHeaders();
                            lm.invalidateLayout(ribbon);
        ...
        // 7. DÉSACTIVATION DES BOUTONS AUX EXTRÉMITÉS
        // Ces états seront mis à jour dynamiquement par le layout
        if (groupIndex == 0) {
            btnLeft.setEnabled(false);
        }
        if (ribbon != null && ribbon.getGroupModel() != null) {
            if (groupIndex == ribbon.getGroupModel().getGroupCount() - 1) {
    ...
    // =========================================================================
    /**
     * Crée un composant pour une valeur nulle. Retourne un label vide.
     *
    ...
    /**
     * Crée un composant pour une chaîne de caractères. Pour les textes longs
     * (>50 caractères), utilise un JTextArea dans un JScrollPane.
     *
    ...
    /**
     * Crée un composant pour un nombre. Les nombres sont alignés à droite pour
    ...
    /**
     * Crée un composant pour un booléen. Utilise une checkbox non éditable avec
     * texte "Oui"/"Non".
     *
    ...
    /**
     * Crée un composant pour une icône. Le label est centré pour un meilleur
     * affichage.
     *
    ...
    /**
     * Crée un composant pour une image AWT. Convertit l'image en ImageIcon puis
     * délègue à createIconComponent.
     *
    ...
    /**
     * Crée un composant pour un fichier. Utilise un bouton cliquable avec le
     * nom du fichier. Le bouton ouvre le dossier parent au clic.
     *
    ...
    /**
     * Crée un composant pour une date java.util.Date. Utilise le formateur de
     * date configuré.
     *
    ...
    /**
     * Crée un composant pour un objet générique. Utilise toString() avec
     * détection du toString() par défaut d'Object.
     *
    ...
    // =========================================================================
    /**
     * Configure un HLabel comme en-tête en utilisant la hiérarchie de
     * configuration.
     *
     * Hiérarchie : Configuration du groupe → Configuration du ruban → Valeurs
     * matérielles
     *
     ...
     *
     * @param label le HLabel à configurer
     * @param headerAlignment la position de l'en-tête (NORTH, SOUTH, WEST,
     * EAST)
     */
    ...
    /**
     * Applique les couleurs de l'en-tête en respectant la hiérarchie de
     * configuration.
     *
     * @param label le HLabel à configurer
    ...
    private void applyHeaderColors(HLabel label, Ribbon ribbon, HRibbonGroup group,
        ...
        // Récupérer le thème actif depuis le Ribbon — null si aucun thème défini
        HRibbonTabsTheme theme = (ribbon != null) ? ribbon.getTheme() : null;

        // RÉSOLUTION DE LA COULEUR DE FOND
        Color backgroundColor;
        if (isSelected) {
            // Groupe spécifique → thème actif → ribbon par défaut → matériel
            if (group != null && group.getHeaderSelectedBackground() != null) {
                backgroundColor = group.getHeaderSelectedBackground();
            } else if (theme != null) {
                backgroundColor = theme.getHeaderBackground();
            } else if (ribbon != null) {
                backgroundColor = ribbon.getDefaultHeaderSelectedBackground();
            } else {
                backgroundColor = new Color(180, 200, 255);
            }
        } else {
            // Groupe spécifique → thème actif → ribbon par défaut → matériel
            if (group != null && group.getHeaderBackground() != null) {
                ...
                backgroundColor = new Color(197, 199, 228);
            }
        }
        ...
        if (group != null && group.getHeaderForeground() != null) {
            ...
            foregroundColor = theme.getHeaderTextColor();
        } else if (ribbon != null) {
            foregroundColor = ribbon.getDefaultHeaderForeground();
        } else {
            foregroundColor = new Color(60, 60, 60);
        }
    ...
    /**
     * Applique la police de l'en-tête en respectant la hiérarchie de
     * configuration.
     *
     * @param label le HLabel à configurer
    ...
    /**
     * Nettoie les composants réutilisables. Utile pour libérer les ressources
