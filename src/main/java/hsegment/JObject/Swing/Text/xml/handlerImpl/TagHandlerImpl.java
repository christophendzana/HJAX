...
public class TagHandlerImpl implements TagHandler {
    ...
    private void validateTagName(TagElement tag) throws HJAXException{
        ...
        if(SyntaxRule.invalidName(tagName)){
            errorHandler.errorHandler(getSourceError(tag.getElement()), "Invalid tag name : "+tagName+". ",
                    "Remove tag name "+tagName, ErrorType.Warning);
        }
        if(!FunctionUtils.verifyStartName(tagName)){
            errorHandler.errorHandler(getSourceError(tag.getElement()), "Invalid first character of tag name : "+tagName+". ",
    ...
    private void validateAttributeName(TagElement tag, String attributeName) throws HJAXException{
        ...
        if(!FunctionUtils.verifyStartName(attributeName)){
            errorHandler.errorHandler(getSourceError(tag.getElement()), "Invalid first character of attribute name : "+attributeName,
        ...
        if(!FunctionUtils.verifyName(attributeName)){
            errorHandler.errorHandler(getSourceError(tag.getElement()), "Invalid attribute name : "+attributeName,
