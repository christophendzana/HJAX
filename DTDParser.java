                ...
                switch(ch){
                    case '('://when parser reach this caracter two case are possible
                        ...
                        if(rootElement == null && !buffer.isEmpty()){
                            rootElement = dtd.defineElement(buffer.toString(), HDTDConstants.MODEL, 
                                    false, false, null, null, null, null);
                            bufferContent = cm = new ContentModel(-1, new HContentModel(rootElement), null);
                            
                            rootElement.content = bufferContent; buffer.delete(0, buffer.length());
                            buffer.delete(0, buffer.length());
                        ...
                        } else if(rootElement != null && cm != null && bufferContent == cm && buffer.isEmpty()){
                            for(; bufferContent != null; bufferContent = bufferContent.next);
                            
                            bufferContent = new ContentModel(-1, new HContentModel(rootElement), null);
                        }else{
                        ...
                        break;
                    case ')':
                        //  Null content Model means '(' character was not been parsed before, so error is thrown
                        if(bufferContent != null)
                            error("DTD.Error", ") caracter is misplaced");
                        if(!buffer.isEmpty() && bufferContent != null && rootElement.type == HDTDConstants.MODEL){
                            Element element = dtd.defineElement(buffer.toString(), -1, false, false, 
                                                                         null, null, null, null);
                            ((HContentModel)bufferContent.content).addElementIndex(element.index);
                            buffer.delete(0, buffer.length());
                        }
                        
                        if(bufferContent != cm){
                            bufferContent = cm;
                        } else if(bufferContent == cm){
                            bufferContent = null; 
                        } else {
                            error("Error.DTD", ") character is misplaced");
                        }
                    ...
                    case ',' :
                        if(!buffer.isEmpty() && bufferContent != null && 
                                (bufferContent.type == ch || bufferContent.type == - 1)){
                            Element element = dtd.defineElement(buffer.toString(), -1, false, false, 
                                                                         null, null, null, null);
                            
                            ((HContentModel)bufferContent.content).addElementIndex(Integer.valueOf(element.index));
                            buffer.delete(0, buffer.length());
                        }else if(buffer.isEmpty() && bufferContent != null && 
                                       (bufferContent.type == ch || bufferContent.type == -1)){
                        //we normally initialiaze content model type with bufferContent.type = ch 
                        }else{
