...
public class HSplitWrapLayout implements LayoutManager2 {
     ...
     * sur l'axe principal. Si false, chaque composant conserve sa
     * preferredSize. False par défaut.
     */
    ...
    public void layoutContainer(Container parent) {
        ...
        // synchronized (parent.getTreeLock(): empêche la modification de la
     ...
     * /**
     * Positionne les composants en lignes horizontales avec retour à la ligne.
     * Si stretch est true, chaque composant s'étire en hauteur pour occuper
    ...
    private void layoutHorizontal(List<Component> components,
        ...
        List<List<Component>> rows = wrapToRows(components, availableW, availableH);

        int y = startY;

        for (List<Component> ligne : rows) {

            int hauteurLigne = stretch ? distributeRowHeight(rows, availableH)
                    : getMaxHeight(ligne);

            int[] largeurs = stretch ? distributeWidth(ligne, availableW) : null;
    ...
    /**
     * Positionne les composants en colonnes verticales avec retour à la
     * colonne. Si stretch est true, chaque composant s'étire en hauteur pour
     * occuper sa portion de la colonne ET en largeur pour occuper toute la
     * largeur de sa colonne — particulièrement utile pour les zones WEST et
     * EAST.
     */
    private void layoutVertical(List<Component> components,
        ...
        List<List<Component>> colonnes = wrapToColumns(components, availableH, availableW);
        ...
        for (List<Component> colonne : colonnes) {
            ...
            int largeurColonne = stretch ? distributeColumnWidth(colonnes, availableW)
                    : getLargeurMax(colonne);

            int[] hauteurs = stretch ? distributeHeight(colonne, availableH) : null;
    ...
    /**
     * Calcule la hauteur équitable allouée à chaque ligne quand stretch est
     * true. Les gaps entre rows sont soustraits avant la distribution.
     *
     * @param rows toutes les rows du layout
     * @param hauteurDispo la hauteur totale disponible
     * @return la hauteur allouée à chaque ligne
     */
    private int distributeRowHeight(List<List<Component>> rows, int hauteurDispo) {
        int n = rows.size();
    ...
    /**
     * Calcule la largeur équitable allouée à chaque colonne quand stretch est
     * true. Les gaps entre colonnes sont soustraits avant la distribution.
     *
     ...
     */
    private int distributeColumnWidth(List<List<Component>> colonnes, int largeurDispo) {
        int n = colonnes.size();
    ...
    public Dimension preferredLayoutSize(Container parent) {
        synchronized (parent.getTreeLock()) {
            ...
            if (direction == WrapDirection.HORIZONTAL) {
                List<List<Component>> rows = wrapToRows(visibles, refW, refH);

                for (List<Component> ligne : rows) {
                    totalH += getMaxHeight(ligne) + vGap;
                    int largeurLigne = getTotalRowWidth(ligne);
                ...
                List<List<Component>> colonnes = wrapToColumns(visibles, refH, refW);

                for (List<Component> colonne : colonnes) {
                    totalW += getLargeurMax(colonne) + hGap;
                    int hauteurColonne = getTotalColumnHeight(colonne);
    ...
    // =========================================================================
    /**
     * Regroupe les composants en lignes en respectant la largeur disponible. Si
     * une nouvelle ligne n'est plus possible faute de hauteur, tous les
     ...
     * @param hauteurDispo la hauteur totale disponible — sert à vérifier si une
     * nouvelle ligne est encore possible
     * @return une liste de lignes, chacune contenant ses composants
     */
    private List<List<Component>> wrapToRows(List<Component> components,
            int largeurDispo,
            int hauteurDispo) {
        ...
        for (Component c : components) {
            ...
            currentRow.add(c);
            xCourant += (currentRow.size() > 1 ? hGap : 0) + largeurComp;
        }
     ...
     * @param largeurDispo la largeur totale disponible — sert à vérifier si une
     * nouvelle colonne est encore possible
     * @return une liste de colonnes, chacune contenant ses composants
     */
    private List<List<Component>> wrapToColumns(List<Component> components,
            int hauteurDispo,
            int largeurDispo) {
        List<List<Component>> columns = new ArrayList<>();
        List<Component> currentColumn = new ArrayList<>();
        int yCourant = 0;
        ...
        for (Component c : components) {
            ...
            currentColumn.add(c);
            yCourant += (currentColumn.size() > 1 ? vGap : 0) + hauteurComp;
        }
     ...
     */
    private int[] distributeWidth(List<Component> ligne, int largeurDispo) {
        int n = ligne.size();
    ...
    /**
     * Distribue la hauteur disponible entre les composants d'une colonne. Les
     * gaps entre composants sont soustraits avant la distribution.
     */
    private int[] distributeHeight(List<Component> colonne, int hauteurDispo) {
        int n = colonne.size();
        int[] result = new int[n];
     ...
     */
    private int getMaxHeight(List<Component> ligne) {
        int max = 0;
        for (Component c : ligne) {
     ...
     */
    private int getTotalRowWidth(List<Component> ligne) {
        int total = 0;
        for (Component c : ligne) {
     ...
     */
    private int getTotalColumnHeight(List<Component> colonne) {
        int total = 0;
        for (Component c : colonne) {
