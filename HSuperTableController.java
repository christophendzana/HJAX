...
public class HSuperTableController {
    ...
    private void handleDoubleClick(int row, int column, MouseEvent e) {
        ...
        // Redirection vers la principale si absorbée
        HSuperDefaultTableModel model = table.getHModel();
        if (model.isAbsorbed(row, column)) {
            java.awt.Point origin = model.findMergeOrigin(row, column);
            if (origin != null) {
                row = origin.x;
                column = origin.y;
            }
    ...
    /**
     * Nettoyage des listeners pour éviter les fuites mémoire. Appeler dispose()
